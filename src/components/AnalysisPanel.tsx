...
import type { AppSettings, AppState, RiskFinding } from '../types';
...
export function AnalysisPanel({ settings, state, localFindings }: AnalysisPanelProps) {
  ...
  async function run() {
    ...
    setResult('');
    try {
      const content = await analyzeWithKimi(settings, state, localFindings);
      setResult(content);
    } catch (err: unknown) {
    ...
    <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800">
      ...
      {!settings.kimiApiKey && (
        <div className="rounded border border-amber-300 bg-amber-50 p-2 text-xs text-amber-800 dark:border-amber-800 dark:bg-amber-900/30 dark:text-amber-100">
          未配置 Kimi API Key。请到「设置」中填入。仅保存在本机浏览器。
        </div>
      )}
      {error && (
